...
export const load: PageServerLoad = async ({ parent, url }) => {
	...
	const filters = [eq(income.userId, userId)];
	if (category) filters.push(eq(income.category, category));
	if (from) filters.push(gte(income.date, new Date(from)));
	if (to) filters.push(lte(income.date, new Date(to)));
